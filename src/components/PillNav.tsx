  ...
  useEffect(() => {
    const layout = () => {
      circleRefs.current.forEach((circle, index) => {
        ...
        if (w === 0 || h === 0) return; // avoid calculation anomalies on SSR layout sync
    ...
    // Document Font Loading Hook to ensure client layout sync runs properly
    ...
    <div className={`pill-nav-container ${className}`}>
      <nav className="pill-nav" aria-label="Primary" style={cssVars}>
        ...
        <div className="pill-nav-items desktop-only" ref={navItemsRef}>
          <ul className="pill-list" role="menubar">
            {items.map((item, i) => (
              <li key={`desktop-item-${i}-${item.href || ''}`} role="none">
                {item.onClick ? (
                  <button
                    type="button"
                    role="menuitem"
      ...
      <div className="mobile-menu-popover mobile-only" ref={mobileMenuRef} style={cssVars}>
        <ul className="mobile-menu-list">
          {items.map((item, i) => (
            <li key={`mobile-item-${i}-${item.href || ''}`} className="w-full flex">
              {item.onClick ? (
                <button
                  type="button"
